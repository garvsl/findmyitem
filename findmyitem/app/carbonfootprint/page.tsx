...
export default function CarbonFootPrint() {
  ...
  useEffect(() => {
    async function getDBdata() {
      try {
        ...
        const todaysData = data.filter(entry => entry.date === today);
  ...
  return (
    <div style={{ textAlign: 'center', marginTop: '50px' }}>
      <Button className="bg-blue-500 text-white hover:bg-blue-700 px-4 py-2 rounded">Carbon Footprint Calculator</Button>
      ...
      <div style={{ marginTop: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {cart.length > 0 && cart.map((entry, index) => (
            <div key={index} style={{
              ...
              <div style={{
                ...
                <p><strong>Unit of Product:</strong> {entry.unit_of_product}</p>
                <p><strong>Number of Units:</strong> {entry.number_of_units}</p>
              </div>
              ...
              <div style={{
                ...
                <p><strong>Carbon Footprint:</strong> {entry.carbon_footprint + entry.unit_of_carbon}</p>
                <p><strong>Sustainable Brand:</strong> {entry.sustainable_brand}</p>
